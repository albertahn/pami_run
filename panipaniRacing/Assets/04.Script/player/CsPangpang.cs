...
public class CsPangpang : MonoBehaviour {
	public GUISkin skin;
	GameObject manager;
	...
	private CsPangpangAni _aniCtrl;
	...
	void Start () {
		...
		_aniCtrl = GetComponent<CsPangpangAni> ();
	...
	void OnCollisionEnter(Collision coll){
		...
		if (coll.transform.tag == "DEAD") {
			//isDead = true;
			//_aniCtrl._animation.CrossFade (_aniCtrl.anim.felt.name, 0.3f);
			StartCoroutine(feltPanelty());
		}
	}
	
	IEnumerator feltPanelty(){
		_aniCtrl._anim.SetBool("setFelt",true);
		//_aniCtrl._animation.CrossFade (_aniCtrl.anim.felt.name, 0.3f);
		isDead = true;
		yield return new WaitForSeconds (2.0f);
		_aniCtrl._anim.SetBool("setFelt",false);
		//_aniCtrl._animation.CrossFade (_aniCtrl.anim.run.name, 0.3f);
		isDead = false;
	}
